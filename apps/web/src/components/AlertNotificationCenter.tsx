...
export const FloatingAlertBanner: React.FC<FloatingAlertBannerProps> = ({
  ...
  return (
    <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 w-[92%] max-w-xl animate-bounceIn">
      <div
        className={`p-3 rounded-2xl backdrop-blur-xl border shadow-2xl flex items-center justify-between gap-3 ${
          isCritical
            ? 'bg-red-950/90 border-red-500/50 text-red-100 shadow-red-950/60 ring-2 ring-red-500/30'
            : 'bg-orange-950/90 border-orange-500/50 text-orange-100 shadow-orange-950/60 ring-2 ring-orange-500/30'
        }`}
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-extrabold uppercase px-1.5 py-0.2 rounded bg-white/20 text-white">
                {alert.severity}
              </span>
              <span className="text-xs font-mono text-slate-300">{alert.timestamp_ist}</span>
            </div>
            <p className="text-xs font-bold text-white truncate mt-0.5">{alert.title}</p>
            <p className="text-[11px] text-slate-200/90 truncate">{alert.message}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-black/30 hover:bg-black/50 text-white border border-white/20 transition-colors"
          >
          ...
          <button
            onClick={onDismiss}
            title="Dismiss"
            className="p-1.5 rounded-lg bg-black/30 hover:bg-black/50 text-slate-300 hover:text-white border border-white/20 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
